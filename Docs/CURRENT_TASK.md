...
System inventory now checks only reviewed official update sources when a project loads, reuses a six-hour local cache and supports a forced refresh through `Neu prüfen`. Available updates expose a confirmed action through the exact catalogued Winget, npm or clean fast-forward Git route and report progress in the project conversation; no program updates automatically.
