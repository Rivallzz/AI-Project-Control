...
Execution preferences are local, project-scoped browser state. Inside the workspace, immediately left of the chat, the operator can enable providers, choose the provider used first and select one model for each provider; the remaining active providers form the visible fallback chain. The server validates the route and model names, records them in the task package and passes them to the router without a prompt-optimization call. Codex models are discovered from the local Codex cache, Ollama chat models from `ollama list`, and Claude exposes its stable CLI aliases. The catalog can be refreshed explicitly without restarting the dashboard. Write mode excludes Hermes/Ollama until its write-safety gate is approved.
