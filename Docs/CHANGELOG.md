...
- Made Graphify runtime discovery prefer an explicit interpreter and then the Windows Python launcher before `PATH` Python, and separated rebuild-tool health from index freshness so an existing stale graph is no longer reported as missing.
